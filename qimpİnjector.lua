...
local TweenService = game:GetService("TweenService") -- EKSİKTİ: Animasyonlar için şart
local UserInputService = game:GetService("UserInputService") -- EKSİKTİ: Klavye/Mouse girdileri için şart

local player = Players.LocalPlayer
local mouse = player:GetMouse() -- EKSİKTİ: Aimlock ve Loading için gerekli
local camera = workspace.CurrentCamera -- EKSİKTİ: Aimlock için gerekli
local pGui = player:WaitForChild("PlayerGui")

-- [TEMİZLİK KISMI - HATA BURADAYDI]
-- İsimleri qimpflex v2 olarak eşitledik ki nil hatası vermesin
if pGui:FindFirstChild("qimpflex v2") then 
	pGui["qimpflex v2"]:Destroy() 
end
if pGui:FindFirstChild("loadingscreen") then 
	pGui.loadingscreen:Destroy() 
end
...
local function StartLoading()
	...
	task.spawn(function()
		...
		-- [[ KAPANISDAKI RENK SORUNU DÜZELTİLDİ ]]
		...
		for _, data in pairs(exitItems) do
			task.spawn(function()
				...
				-- ImageLabel'ın arkasındaki rengi (Background) da kapatıyoruz
				if data.obj:IsA("ImageLabel") then
					...
					-- Kenar çizgisini (Stroke) de kapat
				...
				elseif data.obj:IsA("Frame") then
					...
					for _, child in pairs(data.obj:GetDescendants()) do
						if child:IsA("Frame") then TweenService:Create(child, fadeInfo, {BackgroundTransparency = 1}):Play() end
					end
				end
			end)
		end
...
local function AddPlayerList(parent)
	...
	-- BRING FONKSİYONU (İnsanları senin önüne çeker)
	...
	local function Refresh()
		...
		for _, p in pairs(Players:GetPlayers()) do
			if p ~= player then
				...
				local pName = Instance.new("TextLabel")
				pName.Size = UDim2.new(1, -180, 1, 0) -- İsim alanını butonlar için biraz daralttım
				...
				BringB.Position = UDim2.new(1, -135, 0.5, -12) -- TP'nin soluna yerleşti
				BringB.BackgroundColor3 = Color3.fromRGB(200, 120, 0) -- Farklı olsun diye Turuncu/Amber tonu
				BringB.Text = "BRING"
